...
//			} 
			...
			for (int i = 0; i < columnCount; i++) {
				...
				int columnType = rsmd.getColumnType(i+1); // number 2, date 93, varchar2 12
				 ...
				 if( columnType == 2 ) { // NUMBER
		               if (scale == 0) {   // 정수
		                  System.out.printf("%s\t%s(%d)\n"
		                        , columnName, columnTypeName, precision); 
		               } else {   // 실수
				...
				// 내 풀이
//				String scale = rsmd.getScale(i+1) == 0 ? "": String.valueOf(", "+rsmd.getScale(i+1)) ;
//				System.out.printf("%s\t%s(%d%s)\t%d    \n", 
//													columnName, columnTypeName, precision, scale, columnType );
...
//				rs.getXXX()
				...
				if( columnType == 2 ) { // NUMBER
					int scale = rsmd.getScale(i+1);
		               if (scale == 0) {   // 정수
		            	   System.out.printf("%d\t",rs.getInt(i+1));
		               } else {   // 실수
		            	   System.out.printf("%.2f\t",rs.getDouble(i+1));
		               }
		            }else if( columnType == 12 ) { // VARCHAR2
		            	System.out.printf("%s\t",rs.getString(i+1));
		            }else if(  columnType == 93 ) { // DATE
		            	System.out.printf("%tF\t",rs.getDate(i+1));
		            } 
			} // for
				System.out.println();
			} // while
